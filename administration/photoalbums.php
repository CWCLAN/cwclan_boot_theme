...
if (function_exists('gd_info')) {
    ...
    if (isset($_POST['cancel'])) {
        ...
        if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
            $result = dbquery("SELECT album_title, album_description, album_thumb, album_access, album_order, album_sub FROM " . DB_PHOTO_ALBUMS . " WHERE album_id='" . $_GET['album_id'] . "'");
            if (dbrows($result)) {
                $data = dbarray($result);
        ...
        $albums_opts .= "<option value=''$sel>---</option>\n";
        while ($data = dbarray($result)) {
            $sel = ($data['album_id'] == $album_sub ? " selected='selected'" : "");
            $albums_opts .= "<option value='" . $data['album_id'] . "'$sel>" . $data['album_title'] . "</option>\n";
        }
        ...
        echo "<td class='tbl'><input type='text' name='album_title' value='" . $album_title . "' maxlength='100' class='textbox'/></td>\n";
        echo "</tr>\n<tr>\n";
        echo "<td valign='top' class='tbl'>" . $locale['441'] . "</td>\n";
        echo "<td class='tbl'><textarea name='album_description' cols='60' rows='5' class='textbox'>" . $album_description . "</textarea><br />\n";
        echo display_bbcodes("100%", "album_description", "inputform", "b|i|u|center|small|url|mail|img|quote") . "</td>\n";
        echo "</tr>\n<tr>\n";
        echo "<td class='tbl'>" . $locale['442'] . "</td>\n";
        echo "<td class='tbl'><select name='album_access' class='textbox'>\n" . $access_opts . "</select>\n";
        echo $locale['443'] . "<input type='text' name='album_order' value='" . $album_order . "' maxlength='4' class='textbox'/>\n";
        echo "</td>\n</tr>\n<tr>\n";
        echo "<td class='tbl'>Sub Album</td>\n";
        echo "<td class='tbl'><select name='album_sub' class='textbox'>\n" . $albums_opts . "</select>\n";
        echo "</td>\n</tr>\n<tr>\n";
        echo "<td valign='top' class='tbl'>" . $locale['444'];
        if ((isset($_GET['action']) && $_GET['action'] == "edit") && ($album_thumb && file_exists(BASEDIR . "images/photoalbum/" . $album_thumb))) {
            ...
            echo "</td>\n<td class='tbl'><input type='file' name='album_pic_file' class='textbox'/>";
        }
        echo "</td>\n</tr>\n<tr>\n";
