...
if (file_exists(INFUSIONS . "loa_rates/locale/" . $settings['locale'] . ".php")) {
    include INFUSIONS . "loa_rates/locale/" . $settings['locale'] . ".php";
} else {
    include INFUSIONS . "loa_rates/locale/English.php";
}
...
//--></script>
<script src='" . INFUSIONS . "loa_rates/includes/js/boxover.js' type='text/javascript'></script>");
        ...
        if (!$can_vote) {
            ...
            if (iSUPERADMIN) {
                $vresult = dbquery("SELECT u.user_name, u.user_id FROM fusion_users AS u, fusion_forum_poll_voters AS p
										...
										AND p.forum_vote_user_id = u.user_id");
                if (dbrows($vresult)) {
                    $vresultcount = dbquery("SELECT COUNT(*) AS count FROM fusion_users AS u, fusion_forum_poll_voters AS p
										WHERE p.thread_id = '" . $_GET['thread_id'] . "'
										AND p.forum_vote_user_id = u.user_id");
...
while ($data = dbarray($result)) {
    ...
    $forum_list .= jump_to_forum($data['forum_id']); //subcategories
}
$forum_list .= "</optgroup>\n";
if (iMOD) {
