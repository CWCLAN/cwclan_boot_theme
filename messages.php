...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	echo "<div class='messages_overview'>\n";
	
	// write new message
	echo "<span class='messages_nav_link cwtooltip' title='".$locale['401']."'>\n<a href='".FUSION_SELF."?msg_send=0'><span class='icon icon-pencil' ></a></span>
	</span>\n";
	
	// inbox link
	echo "<span class='messages_nav_link cwtooltip' title='".$locale['402']."' style='white-space:nowrap;font-weight:".($_GET['folder']=="inbox"?"bold":"normal")."'>
	<a href='".FUSION_SELF."?folder=inbox'>
		<span class='icon icon-box-add'></span> <span class='pico_message_info'>&nbsp;".$bdata['inbox_total']."/".($msg_settings['pm_inbox'] != 0 ? $msg_settings['pm_inbox'] : "&infin;")."&nbsp;</span>
	</a>
	</span>\n";
	
	// outbox link
	echo "<span class='messages_nav_link cwtooltip' title='".$locale['403']."' style='white-space:nowrap;font-weight:".($_GET['folder']=="outbox"?"bold":"normal")."'>
	<a href='".FUSION_SELF."?folder=outbox'>
		<span class='icon icon-box-remove'></span> <span class='pico_message_info'>&nbsp;".$bdata['outbox_total']."/".($msg_settings['pm_inbox'] != 0 ? $msg_settings['pm_inbox'] : "&infin;")."&nbsp;</span>
	</a>
	</span>\n";
	
	
	// archive link
	echo "<span class='messages_nav_link cwtooltip' title='".$locale['404']."' style='white-space:nowrap;font-weight:".($_GET['folder']=="archive"?"bold":"normal")."'>
	<a href='".FUSION_SELF."?folder=archive'>
		<span class='icon icon-drawer3'></span> <span class='pico_message_info'>&nbsp;".$bdata['archive_total']."/".($msg_settings['pm_inbox'] != 0 ? $msg_settings['pm_inbox'] : "&infin;")."&nbsp;</span>
	</a>
	</span>\n";
	
	// settings link
	echo "<span class='messages_nav_link cwtooltip' title='".$locale['425']."' style='white-space:nowrap;font-weight:".($_GET['folder']=="options"?"bold":"normal")."'>
	<a href='".FUSION_SELF."?folder=options'>
	...
	$result = dbquery(
		...
		m.message_datestamp, m.message_folder, u.user_id, u.user_avatar, u.user_name, u.user_status
		FROM ".DB_MESSAGES." m
		LEFT JOIN ".DB_USERS." u ON m.message_from=u.user_id
		WHERE (message_to='".$userdata['user_id']."' AND message_from='".$_GET['msg_from']."') OR (message_from='".$userdata['user_id']."' AND message_to='".$_GET['msg_from']."')
